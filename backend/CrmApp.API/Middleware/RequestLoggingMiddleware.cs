...
public class RequestLoggingMiddleware
{
    ...
    public async Task InvokeAsync(HttpContext context, IElasticsearchService elasticsearchService)
    {
        ...
        {
            exception = ex;
            throw; // Rethrow to be handled by GlobalExceptionHandlerMiddleware
        }
        finally
        {
            stopwatch.Stop();

            // Only log if no exception occurred (exceptions are logged by GlobalExceptionHandlerMiddleware)
            if (exception == null)
            {
                // Read response body
                ...
                // Add custom fields for errors (4xx, 5xx)
                if (context.Response.StatusCode >= 400)
                {
                    log.CustomFields["IsError"] = true;
                    log.ErrorMessage = $"HTTP {context.Response.StatusCode}";
                }
            ...
            {
                // Exception occurred, restore response stream
                responseBodyStream.Seek(0, SeekOrigin.Begin);
                await responseBodyStream.CopyToAsync(originalResponseBodyStream);
                context.Response.Body = originalResponseBodyStream;
            }
