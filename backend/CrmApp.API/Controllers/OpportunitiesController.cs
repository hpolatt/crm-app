...
public class OpportunitiesController : BaseController
{
    ...
    [HttpGet]
    public async Task<ActionResult<ApiResponse<PagedResult<OpportunityDto>>>> GetAll(
        [FromQuery] OpportunityFilterQuery filter,
        [FromQuery] PaginationQuery pagination)
    {
        try
        {
            ...
            if (!string.IsNullOrEmpty(filter.SearchTerm))
            {
                filteredOpportunities = filteredOpportunities.Where(o =>
                    o.Title.Contains(filter.SearchTerm, StringComparison.OrdinalIgnoreCase) ||
                    (o.Description != null && o.Description.Contains(filter.SearchTerm, StringComparison.OrdinalIgnoreCase)));
            }

            if (!string.IsNullOrEmpty(filter.Title))
                filteredOpportunities = filteredOpportunities.Where(o => o.Title.Contains(filter.Title, StringComparison.OrdinalIgnoreCase));

            if (!string.IsNullOrEmpty(filter.Stage))
                filteredOpportunities = filteredOpportunities.Where(o => o.Stage == filter.Stage);

            if (filter.AssignedUserId.HasValue)
                filteredOpportunities = filteredOpportunities.Where(o => o.AssignedUserId == filter.AssignedUserId.Value);

            if (filter.CompanyId.HasValue)
                filteredOpportunities = filteredOpportunities.Where(o => o.CompanyId == filter.CompanyId.Value);

            if (filter.ContactId.HasValue)
                filteredOpportunities = filteredOpportunities.Where(o => o.ContactId == filter.ContactId.Value);

            if (filter.AmountMin.HasValue)
                filteredOpportunities = filteredOpportunities.Where(o => o.Value >= filter.AmountMin.Value);

            if (filter.AmountMax.HasValue)
                filteredOpportunities = filteredOpportunities.Where(o => o.Value <= filter.AmountMax.Value);

            if (filter.ProbabilityMin.HasValue)
                filteredOpportunities = filteredOpportunities.Where(o => o.Probability >= filter.ProbabilityMin.Value);

            if (filter.ProbabilityMax.HasValue)
                filteredOpportunities = filteredOpportunities.Where(o => o.Probability <= filter.ProbabilityMax.Value);

            if (filter.ExpectedCloseDateFrom.HasValue)
                filteredOpportunities = filteredOpportunities.Where(o => o.ExpectedCloseDate >= filter.ExpectedCloseDateFrom.Value);

            if (filter.ExpectedCloseDateTo.HasValue)
                filteredOpportunities = filteredOpportunities.Where(o => o.ExpectedCloseDate <= filter.ExpectedCloseDateTo.Value);

            if (filter.IsActive.HasValue)
                filteredOpportunities = filteredOpportunities.Where(o => o.IsActive == filter.IsActive.Value);

            if (filter.CreatedFrom.HasValue)
                filteredOpportunities = filteredOpportunities.Where(o => o.CreatedAt >= filter.CreatedFrom.Value);

            if (filter.CreatedTo.HasValue)
                filteredOpportunities = filteredOpportunities.Where(o => o.CreatedAt <= filter.CreatedTo.Value);
            ...
            var pagedResult = new PagedResult<OpportunityDto>
            {
                Items = opportunities,
            ...
            return Ok(new ApiResponse<PagedResult<OpportunityDto>>
            {
                Success = true,
        ...
        catch (Exception ex)
        {
            ...
            return StatusCode(500, new ApiResponse<PagedResult<OpportunityDto>>
            {
                Success = false,
